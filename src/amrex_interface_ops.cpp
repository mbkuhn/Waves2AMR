...
void
data_amrex::copy_to_fab(int n0, int n1,
...
/*data_amrex::copy_to_fab(int n0, int n1,
  ...
  amrex::Box bx(amrex::IntVect{0, 0, 0}, amrex::IntVect{n0, n1, 0});
  amrex::FArrayBox slab(bx, 3);
  amrex::Array4<amrex::Real> const &data_slab = slab.array();
  amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
    data_slab(i, j, k, 0) = input_vec0[i * n1 + j];
    data_slab(i, j, k, 0) = input_vec1[i * n1 + j];
    data_slab(i, j, k, 0) = input_vec2[i * n1 + j];
  });
  return slab;
}*/